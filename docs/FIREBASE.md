...
- **Firestore indexes** — `firestore.indexes.json` has `fieldOverrides` for `submittedBy.email`, `reviewers.junior`, `reviewers.senior` (ascending, `COLLECTION` + `COLLECTION_GROUP`) and `authorEmails` (`arrayConfig: CONTAINS`, same two scopes). **Deployed and confirmed built** — see "Real bugs found live-testing" below for how a config-looks-right-but-isn't gap here was caught.
...
**To deploy changes to this function going forward:** `firebase deploy --only functions --project bold-d7ff2` for `functions/index.js`; `firebase deploy --only firestore:indexes --project bold-d7ff2` separately for any `firestore.indexes.json` change — the two are never bundled together (lesson from bug 3 above).
