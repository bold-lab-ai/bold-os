...
### Slack App Home dashboard + reviewer assignment (`bold-os#3`) — built 2026-09-12, deployed + debugged + verified live 2026-09-13/14, LIVE

Combines what started as two separate ideas — a slash-command/button to assign reviewers from Slack, and (per Eduardo, same conversation) a personal "what's mine" dashboard in Slack's App Home tab — into one feature, since both need the same App Home + Interactivity infrastructure and the reviewer-picker is naturally one interactive element *inside* the dashboard rather than a separate flow.

**One new HTTP-triggered function, `exports.slackEvents`** (`onRequest`, not a Firestore trigger like the two notification functions above) — doing double duty as both Slack's Events API Request URL, its Interactivity Request URL, and its Select Menus "Options Load URL" (see the reviewer-picker entry below), since the signature-verification step is identical across all three and nothing requires them to be different endpoints. `invoker: 'public'` is set explicitly so `firebase deploy` doesn't stop to ask interactively whether the function may be called unauthenticated — Slack calls it with no Google/Firebase auth of its own, only its own request signature.

- **Request verification.** Every request is checked against Slack's signing scheme (HMAC-SHA256 over `v0:{timestamp}:{raw body}` using a `SLACK_SIGNING_SECRET` secret, `v0=`-prefixed hex) before anything in the body is trusted, with a 5-minute replay window — `verifySlackSignatureRaw`, pure and unit-tested against a hand-computed signature. Without this, anyone on the internet could POST a forged `app_home_opened` event or, worse, a forged reviewer-assignment click.
- **`app_home_opened`** (Events API) → resolves the Slack user id to an email (`emailForSlackId`, the reverse of the existing `slackIdForEmail` — queries `people` by `slackId`), runs four `db.collectionGroup('cards')` queries in parallel (submitted / reviewing-junior / reviewing-senior / authoring via the denormalized `authorEmails`), caps each to the 10 most-recently-updated (sorted in JS, not via Firestore `.orderBy()` — avoids needing composite indexes beyond the single-field `COLLECTION_GROUP` overrides in `firebase/firestore.indexes.json`), and publishes a Block Kit Home view (`views.publish`) with four sections: Registered (with an inline reviewer-picker per card), Reviewing — Junior, Reviewing — Senior, Authoring. Someone not yet in the `people` roster gets a one-line "ask an admin" view instead of an error.
- **Reviewer picker → `external_select` + `block_suggestion`, plus an explicit Clear button.** Each Registered card gets, per role (Junior/Senior), its own full row: an `external_select` — **not `static_select`**: an earlier version shipped with the whole `people` roster as a fixed option list, and Slack silently caps any select menu at 100 options, so with a 274-person roster the picker truncated after the 100th name alphabetically ("stops at letter H") with no error at all, found live-testing, not by inspection — followed immediately by a red (`style: 'danger'`) "✕ Junior"/"✕ Senior" button when that role currently has someone assigned. `external_select` calls back to this same function (`handleBlockSuggestion`/`filterPeopleOptions`) as the viewer types, searching the *whole* roster by name/email substring; `initial_option` is resolved by `initialOptionForEmail` searching the *raw* roster directly rather than a pre-capped list, for the same 100-cap reason. **The Clear button exists because Block Kit's `external_select` has no built-in way for the viewer to unassign a picked value** — confirmed live-testing (an earlier assumption that Slack rendered a native "x" for this was wrong); `parseAssignAction` captures a verb (`assign`|`clear`) from the action_id, and `clear` always writes `''` (`audit-board.html`'s own unassigned sentinel) regardless of the button's payload shape (buttons carry no `selected_option` at all). Each role's select+button live on their own `actions` block (`reviewerPickerBlock` returns an array of two, not one) — the only way to get "the clear button visually paired with its own select" is putting them on the same row, since Block Kit has no column/grid layout; each row's `block_id` is suffixed `:junior`/`:senior` purely to satisfy Slack's view-wide block_id-uniqueness requirement. Picking a name (or clearing) resolves the actor's email, re-checks the exact permission `firebase/firestore.rules` already enforces client-side for reassigning reviewers (`canAssignReviewer` — submitter or PI/admin only, hand-kept in sync with the rule since this write goes through the Admin SDK and bypasses Security Rules entirely), writes `reviewers.{role}` + `updatedAt` (same shape as `onSetReviewer` in `audit-board.html`), then re-publishes the actor's own Home view. A blocked attempt or an unrecognized payload shape is logged and silently ignored — Slack's interactivity has no simple built-in "toast an error" without an extra `response_url` round trip, and a picker that visibly doesn't change is self-evident.
- **`authorEmails: string[]`**, denormalized alongside `authors: {name,email}[]` on every card (`audit-board.html`'s `authorEmailsOf`, wired into `onSaveEdit`/`onAddCard`/`normalizeCard`) — purely so "papers I'm an author on" can be queried at all; Firestore's `array-contains` can't match a partial object inside `authors` directly.
- **Firestore indexes** — `firebase/firestore.indexes.json` has `fieldOverrides` for `submittedBy.email`, `reviewers.junior`, `reviewers.senior` (ascending, `COLLECTION` + `COLLECTION_GROUP`) and `authorEmails` (`arrayConfig: CONTAINS`, same two scopes). **Deployed and confirmed built** — see "Real bugs found live-testing" below for how a config-looks-right-but-isn't gap here was caught.
- **Both directions of the dashboard now live-update** (2026-09-14). Web board → Slack: `audit-board.html`'s `loadCurrentBoardCards()` was a one-time `.get()`, so a reviewer assigned via Slack landed in Firestore correctly but an already-open board tab needed a manual reload to show it. Switched to a persistent `onSnapshot` listener, torn down (`stopWatchingCards()`) on leaving the board (`backToVenuesList`, `popstate` away from venue/card, sign-out) so a stale listener doesn't keep re-rendering an off-screen board or error after sign-out. Slack → web board was fixed, but the *reverse* direction wasn't: a reviewer assigned directly on the web board never told anyone's Slack Home tab anything — the dashboard only ever re-published on `app_home_opened` or right after our own Slack-driven write. Fixed by extending the existing `onCardWritten` trigger: `homeRefreshTargetsForReviewers(before, after)` (pure) decides who needs a refresh whenever `reviewers` changes on a card — the submitter always, plus whoever was newly assigned or un-assigned on either role — and the trigger resolves each to a Slack id and calls the same `handleAppHomeOpened` used everywhere else, independently per person so one failure doesn't block another's refresh.

**Real bugs found live-testing (2026-09-13/14) — all fixed, none were config mistakes on Eduardo's part except where noted:**
1. **Slack's Socket Mode was enabled**, silently routing all Events API / Interactivity traffic over a WebSocket connection instead of the Request URL — `slackEvents` received exactly zero requests despite every other setting (Event Subscriptions verified, `app_home_opened` subscribed, Interactivity URL set, app reinstalled) being genuinely correct. Diagnosed by adding unconditional request-received logging (so "config looks right, zero traffic" became distinguishable from "config looks right, silently failing") and ruling out every other candidate — client cache, workspace app-approval, wrong app — one at a time before finding it. Fixed by turning Socket Mode off; not a code change.
2. **`res.send()` followed by an un-awaited async handler doesn't reliably complete on Cloud Functions v2** — Cloud Run can throttle an instance's CPU right after the response is sent, freezing whatever async work was still in flight. The original "ack fast, handle in the background" pattern (reasonable for Slack's 3-second budget) silently never finished — confirmed via logs showing `handleAppHomeOpened: start` but never its completion line. Fixed by awaiting the real handling *before* responding in both the Events API and Interactivity branches; safe since Slack's budget comfortably covers one Firestore round trip plus one `views.publish` call, and Events API retries on timeout anyway (idempotent — a duplicate Home-tab republish is harmless).
3. **The Firestore index deploy was missed** — `firebase/firestore.indexes.json`'s `fieldOverrides` were committed but `firebase deploy --only functions` doesn't touch Firestore config at all. The dashboard queries failed with `FAILED_PRECONDITION` until `firebase deploy --only firestore:indexes` ran separately; Firestore's own error text (`You can create it here` → `That index is not ready yet` → success) was the ground truth confirming this, not assumption.
4. **The static_select 100-option cap** (see the reviewer-picker bullet above) — real data loss with no error, only caught because Eduardo tried to find a specific person and couldn't.
5. **Two pre-existing test cards had `authors` as plain name strings**, predating the roster/identity migration (guideline 8) — `authorEmails` derived to `[]` for both, so the Authoring dashboard section correctly found nothing (not a bug in the derivation, a real gap in the underlying data). Fixed by hand: matched each author name against the `people` roster and confirmed matches with Eduardo before writing anything (browser-console script, run from his own signed-in session — no Admin SDK key needed, since `hasFullWrite()` as a PI already covers reading every board and writing every card). One card's two ambiguous authors (no roster match, or a nickname not matching any roster name) were deliberately left untouched pending Eduardo's follow-up — no guessing on personal data.

**Verified end-to-end against the real Slack workspace and production Firestore:** Home tab renders all four sections with real data; reviewer-picker search finds people past the old 100-cap; picking a reviewer writes `reviewers.{role}` to the real card, republishes the Home view, and (via the already-live `onCardWritten` trigger) sends the assigned reviewer their DM notification; the web app board reflects a Slack-driven reviewer assignment without a manual reload.

**Still open:** the `authorEmails`/identity gap for the one Plasmax card's two ambiguous authors (pending Eduardo); no rate-limiting on `block_suggestion` (fine at lab scale); Eduardo's own remaining Slack app config was all completed live during this debugging pass (Socket Mode off, `app_home_opened` subscribed, Interactivity + Select Menus Options Load URL both set).

**To deploy changes to this function going forward:** `firebase deploy --only functions --project bold-d7ff2` for `functions/index.js`; `firebase deploy --only firestore:indexes --project bold-d7ff2` separately for any `firebase/firestore.indexes.json` change — the two are never bundled together (lesson from bug 3 above).
...
- **`people` collection**: synced from Slack's `users.list` Web API (274 real, non-bot members, all with emails) via a one-off Admin SDK script — same pattern as `roles`, not automated/scheduled. `firebase/firestore.rules` updated: `people/{personId}` readable by any signed-in lab member, write-denied to clients.
...
- **Public URL is constructed deterministically**, not via `getDownloadURL()`: `https://firebasestorage.googleapis.com/v0/b/{bucket}/o/{encoded path}?alt=media`. No token needed — the object is genuinely public via `firebase/storage.rules`, so `getDownloadURL()`'s share-token mechanism would just be misleading (implies "secret link" when it isn't one).
- **`firebase/storage.rules`**: `venues/{boardId}/feed.ics` is `allow read: if true` (the whole point — calendar apps fetch it unauthenticated); write requires being a PI or admin, checked via `firestore.get(/databases/(default)/documents/roles/pis).data.emails` (and `roles/admins`) — Storage rules can't call Firestore-rules functions directly, so the `hasFullWrite()` check is re-spelled out here against the same underlying data. Everything else in the bucket is deny-all.
...
- **Local project scaffolding:** `firebase.json`, `firebase/firestore.rules` (mirrors the deployed deny-all), `firebase/firestore.indexes.json` (empty — no composite indexes needed yet), `.firebaserc` (default project `bold-d7ff2`) all created in this directory via `firebase_init`.
