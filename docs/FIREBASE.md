...
### Slack App Home dashboard + reviewer assignment (`ml-conference-cycle#3`) — built 2026-09-12 on `feature/slack-app-home`, NOT YET DEPLOYED
...
**One new HTTP-triggered function, `exports.slackEvents`** (`onRequest`, not a Firestore trigger like the two notification functions above) — doing double duty as both Slack's Events API Request URL and its Interactivity Request URL, since the signature-verification step is identical either way and nothing requires them to be different endpoints. `invoker: 'public'` is set explicitly so `firebase deploy` doesn't stop to ask interactively whether the function may be called unauthenticated — Slack calls it with no Google/Firebase auth of its own, only its own request signature.

- **Request verification.** Every request is checked against Slack's signing scheme (HMAC-SHA256 over `v0:{timestamp}:{raw body}` using a new `SLACK_SIGNING_SECRET` secret, `v0=`-prefixed hex) before anything in the body is trusted, with a 5-minute replay window — `verifySlackSignatureRaw`, pure and unit-tested against a hand-computed signature. Without this, anyone on the internet could POST a forged `app_home_opened` event or, worse, a forged reviewer-assignment click.
- **`app_home_opened`** (Events API) → resolves the Slack user id to an email (`emailForSlackId`, the reverse of the existing `slackIdForEmail` — queries `people` by `slackId`), runs four `db.collectionGroup('cards')` queries in parallel (submitted / reviewing-junior / reviewing-senior / authoring via the denormalized `authorEmails`), caps each to the 10 most-recently-updated (sorted in JS, not via Firestore `.orderBy()` — avoids needing composite indexes beyond the single-field `COLLECTION_GROUP` overrides already in `firestore.indexes.json`), and publishes a Block Kit Home view (`views.publish`) with four sections: Registered (with an inline reviewer-picker per card), Reviewing — Junior, Reviewing — Senior, Authoring. Someone not yet in the `people` roster gets a one-line "ask an admin" view instead of an error.
- **Reviewer picker → `block_actions`** (Interactivity) — each Registered card gets two `static_select` elements (Junior/Senior), options built from the whole `people` roster (`personOptionsFor`, capped at Slack's 100-option limit), `initial_option` reflecting the card's current assignee when there is one. Picking a name resolves the actor's email the same way as above, re-checks the exact permission `firestore.rules` already enforces client-side for reassigning reviewers (`canAssignReviewer` — submitter or PI/admin only, hand-kept in sync with the rule since this write goes through the Admin SDK and bypasses Security Rules entirely), writes `reviewers.{role}` + `updatedAt` (same shape as `onSetReviewer` in `audit-board.html`), then re-publishes the actor's own Home view so the picker reflects the change immediately. A blocked attempt or an unrecognized payload shape is logged and silently ignored — Slack's interactivity has no simple built-in "toast an error" without an extra `response_url` round trip, and a picker that visibly doesn't change is self-evident.
...
- **Firestore indexes** — `firestore.indexes.json` gained `fieldOverrides` for `submittedBy.email`, `reviewers.junior`, `reviewers.senior` (ascending, `COLLECTION` + `COLLECTION_GROUP`) and `authorEmails` (`arrayConfig: CONTAINS`, same two scopes). **Not yet verified against a live deploy** — written as a best-effort starting point; Firestore's own missing-index error (with a direct create link) is the authoritative source once the four dashboard queries actually run for the first time.

**Both ack Slack immediately** (`res.status(200).send('')` before any `await`) and do the real Firestore/Slack work after, unawaited by the response — Slack requires a reply within 3 seconds and retries on timeout, and neither rendering a dashboard nor writing a reviewer assignment is guaranteed to be that fast.

**Verified so far:** 40 standalone Node cases covering signature verification (valid/stale/tampered/missing-field), the permission check (submitter/PI/admin/random-person/no-card), action parsing, the Block Kit builders (option sorting/capping, `initial_option` omission vs. `undefined`, section/view shape), and a smoke-test confirming the pre-existing `cardEventsToNotify` behavior is unchanged. **Not yet tested against a live Slack workspace or a real deploy** — no `SLACK_SIGNING_SECRET` has been set yet, and the actual Home-tab rendering/interactivity round trip is only as good as the untested Firestore indexes above.

**Still needed before this can go live:**
1. Eduardo: `firebase functions:secrets:set SLACK_SIGNING_SECRET --project bold-d7ff2` (the app's Signing Secret, from Slack's Basic Information page).
2. `firebase deploy --only functions --project bold-d7ff2` — this prints the new function's URL.
3. Eduardo, in the Slack app config: enable **App Home**; add an **Event Subscription** for `app_home_opened` pointed at that URL (triggers the one-time `url_verification` handshake, already handled); enable **Interactivity & Shortcuts** pointed at the same URL.
4. A real backfill of `authorEmails` on any card documents that predate this field (same shape as the earlier venue `status` backfill) — without it, the Authoring section can't find old papers.
5. Live smoke test: open the Home tab, confirm all four sections render correctly, reassign a reviewer from the picker, confirm the Firestore write and the DM notification (`onCardWritten`, already live) both fire.
