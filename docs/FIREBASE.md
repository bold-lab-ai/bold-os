...
- **Reviewer picker → `external_select` + `block_suggestion`.** Each Registered card gets two `external_select` elements (Junior/Senior) — **not `static_select`**: an earlier version shipped with the whole `people` roster as a fixed option list, and Slack silently caps any select menu at 100 options, so with a 274-person roster the picker truncated after the 100th name alphabetically ("stops at letter H") with no error at all — found live-testing, not by inspection. `external_select` instead calls back to this same function (`handleBlockSuggestion`/`filterPeopleOptions`) as the viewer types, searching the *whole* roster by name/email substring and returning up to 100 live matches — no fixed list, no silent cut. `initial_option` (the card's current assignee, when there is one) is resolved by `initialOptionForEmail` searching the *raw* roster directly rather than a pre-capped option list, for the same reason — the old `initialOptionFor` had the identical 100-cap bug for this lookup. Picking a name resolves the actor's email, re-checks the exact permission `firestore.rules` already enforces client-side for reassigning reviewers (`canAssignReviewer` — submitter or PI/admin only, hand-kept in sync with the rule since this write goes through the Admin SDK and bypasses Security Rules entirely), writes `reviewers.{role}` + `updatedAt` (same shape as `onSetReviewer` in `audit-board.html`), then re-publishes the actor's own Home view. A blocked attempt or an unrecognized payload shape is logged and silently ignored — Slack's interactivity has no simple built-in "toast an error" without an extra `response_url` round trip, and a picker that visibly doesn't change is self-evident.
