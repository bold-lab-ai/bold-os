...
**Design: a pure decision function, then a thin I/O wrapper.** `cardEventsToNotify(before, after, title, boardId, cardId)` computes *what* to notify — a list of `{ emails, headline, url, excludeEmail? }` — with zero I/O (no Firestore, no Slack, no `await` at all); the actual trigger just resolves each event's emails to Slack ids (`slackIdForEmail`, one `people` collection lookup per email — the roster is already synced from Slack's `users.list`, no live `users.lookupByEmail` call needed) and sends it (`chat.postMessage` after `conversations.open`, plain `fetch` against the Slack Web API, no SDK dependency — same minimal-deps preference as the rest of this project). Keeping the decision logic pure is what makes it unit-testable without a live Firestore emulator or a real Slack workspace: 18 standalone cases (every trigger, several multi-event-in-one-write cases, the "no email on file" no-op case, the deep-link URL) verified before this was ever wired to real Slack.
...
**Real deep links + Block Kit, added same day — Eduardo's feedback after the first round of live tests: the messages were plain text with a flat, generic link.** Two changes, on both sides of the wire:

- **`audit-board.html` gained real, bookmarkable URLs.** `history.pushState()`'s third argument (the actual browser URL) was never set before this — Back/Forward worked, but the address bar never changed, so a link *into* the app could only ever land on the front door. `stateUrl(view, boardId, cardId, baseUrl)` builds a hash — `#board=<id>` or `#board=<id>&card=<id>` (a hash, not a real path: this is a static site on GitHub Pages with no server-side routing, so only the fragment can vary without a reload hitting the 404 page) — and `pushNavState()` now passes it as `pushState`'s third argument. `parseDeepLinkHash(hash)` is the reverse, captured once at script load (`pendingDeepLink`) and applied by `applyPendingDeepLink()` the first time `loadInitialData()` has boards loaded to check it against — a venue/card that doesn't exist, and one that exists but isn't visible to this viewer (see `fetchVisibleBoards()` above), get the identical toast, deliberately not distinguishing the two cases.
- **`functions/index.js` mirrors the same hash scheme** (`venueUrl(boardId)`/`cardUrl(boardId, cardId)`, kept in sync by hand with `stateUrl()`, same as `STATUS_LABEL` already was) and now sends real **Slack Block Kit** messages instead of a flat text blob with a pasted URL: `buildMessage(headline, url, buttonLabel)` returns `{ text, blocks }` — a `section` block for the headline (Slack mrkdwn) plus an `actions` block with a real button linking to the specific venue/card. `text` is still set alongside `blocks`, per Slack's own guidance — it's what renders in notification previews and anywhere blocks aren't shown, so it has to stand alone without the button.

**Refined again same day — clearer emoji, and the title itself is a link too (Eduardo's feedback after seeing real messages).** `linkedTitle(text, url)` produces `<url|*text*>`, Slack mrkdwn's bold-link syntax — every headline's paper/venue name goes through this now instead of a bare `*title*`, so the title is clickable in addition to the button below it, not instead of it. Emoji were regrouped into two deliberate families rather than one-per-event-type with no shared logic: 🔔 marks anything that's actually asking the recipient to *decide* something (a venue proposal awaiting approval, a card reaching the PI-approval step) — both used to have unrelated icons (📄, 🖊️) that didn't signal "this needs your action" the same way. Everything else stays a plain FYI icon: 🔍 (a reviewer's sign-off changed on your own card), 📋 (you've been handed a new reviewing task — was 👀, which read more like "watch this" than "here's your assignment"), ➡️ (a plain status change, unchanged), 💬 (a new comment, unchanged — already unambiguous).
