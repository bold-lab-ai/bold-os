...
- [x] **PI and authors on every card — one field.** `card.authors` (string array, at least one required at registration), where **the last author is the PI** (`cardPi()`) — no separate PI field, matching how a paper's own author list usually reads. Collected via dynamic add/remove rows in the registration modal, the last row tagged "PI"; "+ Add author" inserts before it so the PI stays last as co-authors are added. Shown on the detail page (full list) and the card face (PI only, to keep the card compact). Not editable after registration, matching title/email.
